...
public class ThreadManager {
	...
	public Handler runThread(String name, Runnable runnable) {
		if (StringUtils.isNotEmpty(name) == false || runnable == null) {
			Log.i(TAG, "runThread  StringUtil.isNotEmpty(name, true) == false || runnable == null >> return");
			return null;
		}
		...
		if (handler != null) {
			Log.i(TAG, "handler != null >>  destroyThread(name);");
			destroyThread(name);
		}
		...
		Log.i(TAG, "runThread  added name = " + name + "; threadMap.size() = " + threadMap.size() + "\n");
		return handler;
	}
